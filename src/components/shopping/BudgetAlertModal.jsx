...
const BudgetAlertModal = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="480px" hideHeader={true}>
      <div
        className={`p-10 flex flex-col items-center text-center ${bgClass} rounded-2xl relative overflow-hidden`}
      >
        ...
        <div className="mb-6 animate-bounce">
          <AlertTriangle
            size={64}
            className={isCritical ? "text-white" : "text-gray-900"}
            strokeWidth={2.5}
          />
        </div>
        ...
        {/* Title */}
        <h2
          className={`text-2xl font-black uppercase tracking-tight mb-4 ${isCritical ? "text-white" : "text-gray-900"}`}
        >
        ...
        {/* Message */}
        <p
          className={`text-sm font-bold mb-10 px-4 leading-relaxed ${isCritical ? "text-white/90" : "text-gray-700"}`}
        >
          {isCritical ? (
            <>
              <span className="text-black block mb-1">Attention:</span>
              You have reached {percentage}% of your budget limit. Please
              increase your budget or stop further transactions.
            </>
          ) : (
            <>
              You've used {percentage}% of your total budget.
              <br />
              You might want to slow down your spending.
            </>
          )}
        </p>
