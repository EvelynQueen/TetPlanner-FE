  ...
  return (
    <div className="card p-0 bg-white shadow-sm rounded-xl overflow-hidden mt-8 relative">
      <div className="px-6 py-4 border-b border-gray-50 text-left">
        <h3 className="text-lg font-bold text-gray-800">Detailed Shopping List</h3>
      </div>
              ...
              <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-left">
                  <span className="px-2 py-1 rounded text-[10px] font-bold uppercase" style={{
                    backgroundColor: item.categoryName === 'Food' ? '#FEF3C7' : item.categoryName === 'Gift' ? '#F3E8FF' : item.categoryName === 'Decoration' ? '#FCE7F3' : '#F3F4F6',
                    color: item.categoryName === 'Food' ? '#D97706' : item.categoryName === 'Gift' ? '#7E22CE' : item.categoryName === 'Decoration' ? '#BE185D' : '#6B7280'
                  }}>
                    {item.categoryName || 'General'}
                  </span>
                </td>
                <td className="px-6 py-4 text-gray-600 text-left">${item.price.toLocaleString()}</td>
                <td className="px-6 py-4 text-gray-600 text-left">{item.quantity}</td>
                ...
                <td className="px-6 py-4 text-left">
                  <button 
                    ...
                    <div className={`w-1.5 h-1.5 rounded-full ${item.isChecked ? "bg-green-500" : "bg-orange-500"}`}></div>
